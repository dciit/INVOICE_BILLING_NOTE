...
const EBilling_DetailModalVendor: React.FC<EBilling_DetailModalVendorProps> = ({ open, onClose, invoiceDetail, refreshData }) => {
    ...
    const fetchData = async () => {
        try {
            const res = await service.PostReportVendorDetail({ documentNo: invoiceDetail.documentno });
    ...
    const RejectBeforeConfirmbilling = async () => {
        try {
            setLoading(true);
            ...
            await service.PostRejectBeforeConfirmbilling({
                documentNo: DataSource[0]?.documentno,
                receiveBy: auth.incharge.trim()
            });
    ...
    return (
        ...
        >
            ...
            {/* {
                JSON.stringify(invoiceDetail[0]?.documentno)
            } */}
            <div
                style={{
                    position: "relative",
